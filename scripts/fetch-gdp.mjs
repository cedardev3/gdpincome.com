 ...
 *
 * Primary sources:
 * - USA: BEA GDP by Industry (published quarterly value added)
 ...
 * Primary: BEA, StatCan, ABS, Eurostat, World Bank industry; OECD for JP/GB (+ fallback).
...
async function main() {
  ...
  console.log("Fetching USA from BEA GDP by Industry…");
  let usa;
  try {
  ...
  const world = {
    ...
    sources: [
      { label: "BEA GDP by Industry", url: "https://apps.bea.gov/iTable/?reqid=1603&step=2&Categories=GDPxInd&isURI=1" },
  ...
  const meta = {
    ...
    sources: {
      bea: {
        dataset: "BEA GDP by Industry, Value Added by Industry",
        countries: ["USA"],
        url: "https://apps.bea.gov/iTable/?reqid=1603&step=2&Categories=GDPxInd&isURI=1",
        measure: "Value added by industry, current $, SAAR, published summary plus one finer group",
      },
      statcan: {
